...
while : ; do
	...
	if [[ "$storage" = /dev/nvme* ]] || [[ "$storage" = /dev/mmcblk* ]]; then
		storagePartitions=([1]="$storage"p1 [2]="$storage"p2 [3]="$storage"p2)
		break
...
grubPerformanceOptions="rootflags=atgc rw mitigations=off transparent_hugepage=madvise nowatchdog quiet loglevel=0 amd_pstate=guided"
...
if [ "$boot" = bios ] || [ "$boot" = efi ]; then
	...
	parted -a optimal -s "$storage" mkpart primary 1024MiB 2048MiB
	...
	if [ "$encrypt" = y ]; then
		...
		echo "$encpass" | cryptsetup --iter-time 5000 --use-urandom --type luks2 --cipher aes-xts-plain64 --key-size 512 --pbkdf argon2id luksFormat "${storagePartitions[3]}"
		...
		if [ "$deviceUsesSSD" = yes ]; then
			echo "$encpass" | cryptsetup --allow-discards --perf-no_read_workqueue --perf-no_write_workqueue --persistent open "${storagePartitions[3]}" cryptroot
			...
			echo "$encpass" | cryptsetup open "${storagePartitions[3]}" cryptroot
		fi
	else
		#If encryption is not set make this set to ${storagePartitions[2]}
		rootTargetDisk="${storagePartitions[3]}"
	fi
	...
	if [ "$filesystem" = ext4 ] ; then
		...
		dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
		--title "Patitioning Disk" \
		--prgbox "Formatting root partition" "mkfs.f2fs -f -l ArchRoot -O extra_attr,inode_checksum,sb_checksum,compression,flexible_inline_xattr,inode_crtime $rootTargetDisk" "$HEIGHT" "$WIDTH"
		...
		dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
		--title "Patitioning Disk" \
		--prgbox "Formatting root partition" "bcachefs format -f -L ArchRoot --compression=zstd --background_compression=zstd --discard $rootTargetDisk" "$HEIGHT" "$WIDTH"
	fi
	...
	if [ "$filesystem" = btrfs ] ; then
		...
		mount -o compress_algorithm=lzo-rle,compress_chksum,atgc,gc_merge,inline_xattr,flush_merge,discard,lazytime "$rootTargetDisk" /mnt
	elif [ "$filesystem" = bcachefs ] ; then
	...
	mount --mkdir "${storagePartitions[1]}" /mnt/efi

	#Mount and partition the boot partition
	dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
	--title "Patitioning Disk" \
	--prgbox "Formatting boot partition" "mkfs.xfs ${storagePartitions[2]}" "$HEIGHT" "$WIDTH"
	mount --mkdir "${storagePartitions[2]}" /mnt/boot
fi
clear
...
--prgbox "Installing base-devel package group and kernel" "arch-chroot /mnt pacman -Syy && arch-chroot /mnt pacman -S base-devel linux-cachyos-rc-lto linux-cachyos-rc-lto-headers amd-ucode linux-firmware f2fs-tools xfsprogs mkinitcpio lzop lz4 grub efibootmgr dosfstools mtools btrfs-progs dbus-broker dbus-broker-units --noconfirm" "$HEIGHT" "$WIDTH"
...
if [ "$cpuVendor" = Intel ]; then
	...
	--prgbox "Installing AMD Microcode" "arch-chroot /mnt pacman -S amd-ucode nano nano-syntax-highlighting zsh zsh-completions bc bison flex ccache cmake extra-cmake-modules ninja meson musl mesa-git llvm llvm-libs compiler-rt clang lld lldb polly libunwind openmp libc++ libc++abi --noconfirm" "$HEIGHT" "$WIDTH"
fi
clear
...
--prgbox "Installing desktop environment" "arch-chroot /mnt pacman -Syy && arch-chroot /mnt pacman -S --needed wget nano nm-connection-editor networkmanager grub-customizer gparted gnome-disk-utility lzip lzop cpio zip unzip htop jre-openjdk mousepad papirus-icon-theme ttf-ubuntu-font-family ttf-ibm-plex bash-completion ffmpeg openssh gvfs-mtp cpupower ttf-dejavu ttf-liberation noto-fonts fastfetch nano-syntax-highlighting imagemagick freetype2 rsync acpi noto-fonts-emoji unrar earlyoom xorg-xrandr gpart pinta irqbalance xf86-video-amdgpu firefox hdparm usbutils systembus-notify kitty kernel-modules-hook mlocate gtk-engine-murrine xorg-xkill f2fs-tools xorg-xhost exfatprogs profile-sync-daemon reflector xorg-server xdg-user-dirs xdg-desktop-portal xdg-desktop-portal-gtk mold p7zip xorg-xauth xorg-mkfontscale xorg-fonts gnome gdm --noconfirm" "$HEIGHT" "$WIDTH"
clear
#Additional aurmageddon packages
dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
--title "Installing additional desktop software" \
--prgbox "Installing Aurmageddon packages" "arch-chroot /mnt pacman -S trizen ttf-symbola pokemon-colorscripts-git arch-silence-grub-theme-git bibata-cursor-translucent usbimager pacman-cleanup-hook ttf-unifont lscolors-git zramswap preload pacman-updatedb-hook needrestart --noconfirm" "$HEIGHT" "$WIDTH"
clear
...
--prgbox "Enabling core system services" "arch-chroot /mnt systemctl enable NetworkManager systemd-timesyncd ctrl-alt-del.target irqbalance earlyoom zramswap linux-modules-cleanup logrotate.timer fstrim.timer archlinux-keyring-wkd-sync.timer gdm" "$HEIGHT" "$WIDTH"
clear
...
if [ "$chassisType" = laptop ] || [ "$chassisType" = tablet ] || [ "$acpiBattery" = yes ] || [ "$sysBattery" = yes ]; then
	...
	#Install thermald if CPU is Intel
	if [ "$cpuVendor" = Intel ]; then
		dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
		--title "Intel Laptop Found" \
		--prgbox "Setting up thermald" "arch-chroot /mnt pacman -S thermald --noconfirm && arch-chroot /mnt systemctl enable thermald.service" "$HEIGHT" "$WIDTH"
	fi
	clear
fi
...
if [ "$boot" = efi ]; then
	if [ "$bootArch" = 64 ]; then
		...
		--prgbox "Installing grub for UEFI" "arch-chroot /mnt grub-install --target=x86_64-efi --efi-directory=/efi --bootloader-id=GRUB --removable --recheck" "$HEIGHT" "$WIDTH"
	else
		dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
		--title "GRUB installation" \
